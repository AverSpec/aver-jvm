...
class SelfTest {
    ...
    @Test
    fun `domain collects markers`() = s.run { ctx ->
    ...
    @Test
    fun `domain markers queryable`() = s.run { ctx ->
        ctx.given(AverCoreDomain.defineDomain, DomainSpec(
            name = "queryable",
    ...
    @Test
    fun `complete adapter builds`() = s.run { ctx ->
        ctx.given(AverCoreDomain.defineDomain, DomainSpec(
            name = "complete",
        ...
        ctx.act(AverCoreDomain.createAdapter, AdapterSpec(protocolName = "unit"))
        ctx.then(AverCoreDomain.adapterIsComplete, CompletenessCheck(missing = emptyList()))
    }

    @Test
    fun `incomplete adapter reports missing`() = s.run { ctx ->
        ctx.given(AverCoreDomain.defineDomain, DomainSpec(
            name = "incomplete",
    ...
    @Test
    fun `action dispatch records trace`() = s.run { ctx ->
        ctx.given(AverCoreDomain.defineDomain, DomainSpec(
            name = "traced",
            actions = listOf("do_thing"),
            queries = emptyList(),
        ...
        ctx.act(AverCoreDomain.callOperation, OperationCall(markerName = "do_thing", payload = "hello"))
        ctx.then(AverCoreDomain.traceHasEntry, TraceCheck(index = 0, category = "when", status = "pass"))
    }

    @Test
    fun `multiple operations build trace`() = s.run { ctx ->
        ctx.given(AverCoreDomain.defineDomain, DomainSpec(
            name = "multi-trace",
        ...
        ctx.given(AverCoreDomain.createAdapter, AdapterSpec())
        ctx.act(AverCoreDomain.callOperation, OperationCall(markerName = "step_one"))
        ctx.act(AverCoreDomain.callOperation, OperationCall(markerName = "step_two"))
    ...
    @Test
    fun `when rejects assertions`() = s.run { ctx ->
        ctx.given(AverCoreDomain.defineDomain, DomainSpec(
            name = "proxy-test",
            actions = listOf("do_thing"),
            queries = emptyList(),
    ...
    @Test
    fun `then rejects actions`() = s.run { ctx ->
        ctx.given(AverCoreDomain.defineDomain, DomainSpec(
            name = "proxy-test-2",
            actions = listOf("do_thing"),
            queries = emptyList(),
    ...
    @Test
    fun `query rejects actions`() = s.run { ctx ->
        ctx.given(AverCoreDomain.defineDomain, DomainSpec(
            name = "proxy-test-3",
            actions = listOf("do_thing"),
            queries = listOf("get_thing"),
