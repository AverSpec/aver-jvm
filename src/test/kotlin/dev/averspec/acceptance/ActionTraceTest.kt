...
class ActionTraceTest {
    ...
    @Test
    fun `records complete trace across multiple operations`() {
        s.test("trace full") { ctx ->
    ...
    @Test
    fun `records failure status when assertion fails`() {
        s.test("trace fail") { ctx ->
            ctx.given(AverCoreDomain.defineDomain, DomainSpec(
                name = "trace-fail",
    ...
    @Test
    fun `records categorized trace with given when then`() {
        s.test("trace categorized") { ctx ->
            ctx.given(AverCoreDomain.defineDomain, DomainSpec(
    ...
    @Test
    fun `trace is empty before any operations`() {
        s.test("trace empty") { ctx ->
            ctx.given(AverCoreDomain.defineDomain, DomainSpec(
                name = "trace-empty",
                actions = listOf("noop"),
                queries = emptyList(),
