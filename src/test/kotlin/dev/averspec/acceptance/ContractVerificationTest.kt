...
class ContractVerificationTest {
    ...
    fun `extract contract from passing test with static telemetry`() = s.run { ctx ->
        ...
        ctx.act(AverCoreDomain.runContractOperations, Unit)
        ctx.act(AverCoreDomain.extractAndWriteContract, Unit)
        ctx.act(AverCoreDomain.loadAndVerifyContract, ContractTraceSpecPayload(
            spans = listOf(mapOf(
